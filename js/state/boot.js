"use strict";
window.rocky.state.boot = {
	soundArray: [],
	preload: function(){
		
		
		
		// set world size
		this.game.world.setBounds(0, 0, mt.data.map.worldWidth, mt.data.map.worldHeight);
		
		this.enableScaling();
		
		//init mt helper
		mt.init(this.game);
		
		//set background color - true (set also to document.body)
		mt.setBackgroundColor(true);
		
		// load assets for the Loading group ( if exists )
		mt.loadGroup("Loading");
		// sounds
		window.rocky.game.load.audio("music", ["music.mp3"]);
	},
	create: function(){
		// add all game states
		for(var stateName in window.rocky.state){
			this.game.state.add(stateName, window.rocky.state[stateName]);
		}
		
		// sounds
		this.soundArray[0] = window.rocky.game.add.audio("music", 0.3);
		this.soundArray[0].play();
		
		// goto load state
		this.game.state.start("load");
	},
	enableScaling: function(){
		var game = this.game;
		game.scale.parentIsWindow = (game.canvas.parentNode == document.body);
		game.scale.scaleMode = Phaser.ScaleManager[mt.data.map.scaleMode];
	}
};